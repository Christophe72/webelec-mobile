...
import type { ChuteTensionInputs, ChuteTensionResult, CircuitType, Tension, InstallationType } from '@/types/dto/calculateur';
...
interface ChuteTensionCalculatorProps {
  ...
  onChange: <K extends keyof ChuteTensionInputs>(field: K, value: ChuteTensionInputs[K]) => void;
}

export function ChuteTensionCalculator({ inputs, onChange }: ChuteTensionCalculatorProps) {
  ...
  }, [inputs.section, inputs.longueur, inputs.courant, inputs.tension, inputs.typeCircuit, inputs.typeInstallation]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value) => onChange('tension', parseInt(value) as Tension)}
          >
        ...
        {/* Type d'installation */}
        <div className="space-y-2">
          <Label htmlFor="typeInstallation-chute" className="text-base font-medium">
            Type d'installation
          </Label>
          <Select
            value={inputs.typeInstallation || 'neuf'}
            onValueChange={(value) => onChange('typeInstallation', value as InstallationType)}
          >
            <SelectTrigger id="typeInstallation-chute" className="h-12 text-base">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
      ...
      {!result && (inputs.section === 0 || inputs.longueur === 0 || inputs.courant === 0) && (
        <div className="rounded-lg border border-dashed border-zinc-300 p-4 text-center text-sm text-muted-foreground dark:border-zinc-700">
          Remplissez la section, la longueur et le courant pour voir le résultat
        </div>
      )}
