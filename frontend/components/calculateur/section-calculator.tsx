...
import type { SectionInputs, SectionResult, CircuitType, Tension, InstallationType } from '@/types/dto/calculateur';
...
interface SectionCalculatorProps {
  ...
  onChange: <K extends keyof SectionInputs>(field: K, value: SectionInputs[K]) => void;
  onUseResult?: (sectionRecommandee: number) => void;
}

export function SectionCalculator({ inputs, onChange, onUseResult }: SectionCalculatorProps) {
  ...
  }, [inputs.courant, inputs.longueur, inputs.tension, inputs.typeCircuit, inputs.typeInstallation]);
  ...
  const canUseResult = result && (result.status === 'ok' || result.status === 'limite');
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Ex: 16"
          />
          <p className="text-sm text-muted-foreground">Intensité nominale du circuit</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="Ex: 25"
          />
          <p className="text-sm text-muted-foreground">Distance aller simple (tableau → point d&#39;utilisation)</p>
        </div>
