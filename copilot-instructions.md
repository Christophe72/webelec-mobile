...
✅ Structure idéale pour WebElec SaaS
...
│   ├── lib/                      # utilitaires front (fetchers, helpers)
│   ├── hooks/                    # hooks client uniquement
│   ├── public/                   # images, logos WebElec
│   ├── styles/                   # CSS/Tailwind config
│   ├── prisma/                   # (si front gère une DB locale)
│   ├── .env.local
│   └── package.json
│
...
│   │   ├── model/                # entités JPA (Societe, Client…)
│   │   ├── dto/                  # DTOs pour API
│   │   └── config/               # sécurité, cors, jwt
│   ├── src/main/resources/
│   │   └── application.yml
│   ├── target/                   # build maven (automatique)
│   ├── pom.xml
│   └── Dockerfile
│
├── database/
│   ├── migrations/               # scripts SQL Flyway/Liquibase
│   ├── seeds/                    # jeux de données (chantiers test, RGIE…)
│   └── docker/                   # config Postgres + PGAdmin
│
├── iot/                          # ESP32, MQTT, Node-RED, firmwares
│   ├── esp32/                    # code Arduino/Espressif
│   ├── mqtt-topics.md            # liste des topics officiels WebElec
│   ├── node-red/                 # flows Node-RED
│   └── schemas/                  # formats JSON des mesures
│
├── docs/                         # Documentation métier
│   ├── rgie/                     # extraits RGIE structurés (JSON/MD)
│   ├── api/                      # doc OpenAPI/Swagger du backend
│   ├── archi/                    # schémas UML, PlantUML, mermaid
│   ├── guides/                   # guides utilisateur / dev
│   └── README.md
│
├── scripts/                      # scripts pratiques
│   ├── safe-push.ps1             # Git push sécurisé
│   ├── init-dev.ps1              # setup dev environnement
│   ├── deploy-vps.sh             # déploiement automatique
│   └── backup-db.sh              # backup Postgres
│
├── deployments/
│   ├── docker-compose.yml        # orchestration front + back + db
│   ├── k8s/                      # manifestes Kubernetes (si futur)
│   └── nginx.conf                # reverse proxy pour VPS
│
├── .github/
│   ├── workflows/                # CI/CD GitHub Actions
│   ├── copilot-instructions.md   # tes règles d’IA strictes
│   └── ISSUE_TEMPLATE.md
│
├── LICENSE
├── README.md
└── .env                          # variables globales (jamais commit)
